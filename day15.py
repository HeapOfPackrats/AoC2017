...
def main():
    ...
    A = makeGenerator(puzzleInput["A"], 16807)
    B = makeGenerator(puzzleInput["B"], 48271)
    print("There are {} matching pairs".format(judgeVals(A, B, 40000000)))
...
def makeGenerator(startVal, factor):
    val = startVal
    def generate():
        nonlocal val
        ...
        return val - 0x7fffffff if val >> 31 else val
    return generate
